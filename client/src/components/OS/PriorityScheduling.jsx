...
export default function PriorityScheduling() {
  ...
  // ✅ Add process
  const addProcess = () => {
    if (!pid || burst === '' || arrival === '' || priority === '') return;
    if (burst < 0 || arrival < 0 || priority < 0) return alert("No negative values allowed!");
    setProcesses([...processes, { pid, burst: parseInt(burst), arrival: parseInt(arrival), priority: parseInt(priority) }]);
    setPid('');
    setBurst('');
    setArrival('');
    setPriority('');
  ...
  // ✅ Animate and calculate averages
  const animateTimeline = (steps, results) => {
    setTimeline(steps);
  ...
  const runNonPreemptive = () => {
    ...
    let steps = [];
    let results = [];
    let readyQueue = [...processes].sort((a, b) => a.arrival - b.arrival);
    let completed = [];

    while (completed.length < processes.length) {
      const available = readyQueue.filter(p => p.arrival <= time && !completed.includes(p));

      if (available.length === 0) {
        const next = readyQueue.find(p => !completed.includes(p));
        steps.push({ pid: 'Idle', start: time, end: next.arrival });
        time = next.arrival;
        continue;
      }
      ...
      results.push({ pid: p.pid, arrival: p.arrival, burst: p.burst, priority: p.priority, CT, TAT, WT });

      completed.push(p);
      time = end;
    }

    animateTimeline(steps, results);
  };
  ...
  const runPreemptive = () => {
    ...
    while (remaining.some(p => p.remaining > 0)) {
      ...
      if (available.length === 0) {
        if (currentPid !== 'Idle') {
          if (currentPid) steps.push({ pid: currentPid, start: segmentStart, end: time });
          currentPid = 'Idle';
          segmentStart = time;
        }
        time++;
    ...
    for (let i = 0; i < steps.length; i++) {
      if (merged.length && merged[merged.length - 1].pid === steps[i].pid) {
        merged[merged.length - 1].end = steps[i].end;
      } else {
        merged.push({ ...steps[i] });
      }
    }

    animateTimeline(merged.filter(s => s.start !== s.end), results);
  };

  return (
    <div className="p-6 flex flex-col items-center bg-gray-50 min-h-[calc(100vh-56px)]">
      <h1 className="text-2xl font-bold mb-4">Priority Scheduling</h1>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <input className="border px-3 py-1 rounded" placeholder="Priority (Lower = Higher)" type="number" min="0" value={priority} onChange={e => setPriority(Math.max(0, e.target.value))} />
        ...
        <ul className="space-y-1">
          {processes.map((p, i) => (
            <li key={i} className="bg-white px-3 py-1 rounded shadow">
              {p.pid} | BT: {p.burst} | AT: {p.arrival} | Priority: {p.priority}
            </li>
          ))}
        </ul>
      </div>
            ...
            <tbody>
              {resultTable.map((row, i) => (
                <tr key={i} className="text-center">
                  ...
                  <td className="border px-3 py-1">{row.priority}</td>
