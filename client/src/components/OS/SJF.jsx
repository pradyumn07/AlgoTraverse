...
export default function SJF() {
  ...
  const runPreemptive = () => {
    ...
    let steps = [];
    let results = [];
    let remaining = processes.map(p => ({ ...p, remaining: p.burst, start: null, end: null }));
    const totalBurst = processes.reduce((sum, p) => sum + p.burst, 0);

    let currentPid = null;
    let segmentStart = 0;

    while (time < totalBurst + Math.min(...processes.map(p => p.arrival))) {
  ...
  return (
    <div className="p-6 flex flex-col items-center bg-gray-50 min-h-[calc(100vh-56px)]">
      <h1 className="text-2xl font-bold mb-4">SJF Scheduling</h1>
